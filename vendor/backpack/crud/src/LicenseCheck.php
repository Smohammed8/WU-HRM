...
trait LicenseCheck
{
    ...
    private function checkLicenseCodeExists()
    {
        ...
        // don't show notice bubble on hrm.ju.edu.et
        if (in_array($_SERVER['REMOTE_ADDR'] ?? [], ['hrm.ju.edu.et'])) {
            return;
        }
       ...
       if (config('app.debug') == 'false' && config('app.env') == 'production') {
        ...
        if (! $this->validCode(config('backpack.base.license_code'))) {
            \Alert::add('warning', "<strong>Invalide license!</strong> 
            <a target='_blank' class='alert-link text-white' href='#'>Contact Developer</a> to hide this message.");
        }
    }
