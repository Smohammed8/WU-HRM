          ...
          <div class="col-sm-6">
            @if ( $crud->buttons()->where('stack', 'top')->count() ||  $crud->exportButtons())
              <div class="d-print-none {{ $crud->hasAccess('create')?'with-border':'' }}">
                ...
                @if($placementRound->status > \App\Constants::PLACEMENT_ROUND_STATUS_OPENED  && $placementRound->status < \App\Constants::PLACEMENT_ROUND_STATUS_APPROVED )
                    <a href="{{ route('placement.reset',['placement_round'=>$placementRound->id]) }}" class="btn btn-outline-primary" data-style="zoom-in">
                ...
                @if($placementRound->status == \App\Constants::PLACEMENT_ROUND_STATUS_OPENED  )
                    <a href="{{ route('compute_rank',['placement_round'=>$placementRound->id]) }}" class="btn btn-outline-primary" data-style="zoom-in">
                        <span class="ladda-label">
                        Compute Rank
                    </span>
                    </a>
                ...
                @if($placementRound->status == \App\Constants::PLACEMENT_ROUND_STATUS_PLACED)
                    <a href="{{ route('placement.approve',['placement_round'=>$placementRound->id]) }}" class="btn btn-outline-primary" data-style="zoom-in">
                        <span class="ladda-label">
                            Approve Placement
                        </span>
                    </a>
                ...
                @if($placementRound->status == \App\Constants::PLACEMENT_ROUND_STATUS_APPROVED)
                    <a href="{{ route('placement.close',['placement_round'=>$placementRound->id]) }}" class="btn btn-outline-primary" data-style="zoom-in">
                        <span class="ladda-label">
                            Close Placement
                        </span>
                    </a>
